 ...
 * Runs the embedding model and all heavy computation off the main thread.
 */
...
type WorkerResponse =
  ...
  | { type: "result"; id: number; words: WordEntry[] }
...
async function handleSearch(req: WorkerRequest): Promise<void> {
  const send = (msg: WorkerResponse) => self.postMessage(msg)

  try {
    // Step 1: Load model
    send({ type: "progress", id: req.id, progress: { stage: "loading-model", percent: 0 } })
    await getEmbedder()
    send({ type: "progress", id: req.id, progress: { stage: "loading-model", percent: 100 } })

    // Step 2: Embed words
    send({ type: "progress", id: req.id, progress: { stage: "embedding-words", percent: 0 } })

    const suitable = req.words.filter((w) => w.word.length >= 3 && w.word.length <= 15)
    ...
    if (uncachedTexts.length > 0) {
      const vectors = await embedTexts(uncachedTexts, (done, total) => {
        send({
          type: "progress",
          id: req.id,
          progress: { stage: "embedding-words", percent: Math.round((done / total) * 100) },
        })
      })
    ...
    send({ type: "progress", id: req.id, progress: { stage: "embedding-words", percent: 100 } })

    // Step 3: Embed theme and rank
    send({ type: "progress", id: req.id, progress: { stage: "searching", percent: 0 } })

    const [themeVector] = await embedTexts([req.theme])
    ...
    send({ type: "progress", id: req.id, progress: { stage: "searching", percent: 100 } })
    send({ type: "result", id: req.id, words: scored.slice(0, req.count).map((s) => s.entry) })
  } catch (err) {
